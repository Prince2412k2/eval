...
class SemanticChunker:
    ...
    def _group_units_into_chunks(
        self, units: List[Dict], page: int, start_idx: int
    ) -> List[Chunk]:
        """
        ...
        for i, unit in enumerate(units):
            ...
            if unit.get("priority") == "critical":
                if unit_size > self.max_chunk_size:
                    if current_units:
                        chunks.append(
                            self._create_chunk(current_units, page, chunk_idx)
                        )
                        chunk_idx += 1
                        current_units = []
                        current_size = 0
            ...
            if unit.get("keep_together"):
                if unit_size > self.max_chunk_size:
                    ...
                    for sub_unit in sub_units:
                        ...
                        if (
                            current_size + sub_size > self.max_chunk_size
                            and current_units
                        ):
                            chunks.append(
                                self._create_chunk(current_units, page, chunk_idx)
                            ...
                            overlap_unit = self._create_overlap_context(current_units)
                            current_units = (
                                [overlap_unit, sub_unit] if overlap_unit else [sub_unit]
                            )
                            current_size = sum(len(u["content"]) for u in current_units)
                        else:
            ...
            for sent_unit in sentences:
                ...
                if current_size + sent_size > self.max_chunk_size and current_units:
                    ...
                    overlap_unit = self._create_overlap_context(current_units)
                    current_units = (
                        [overlap_unit, sent_unit] if overlap_unit else [sent_unit]
                    )
                    current_size = sum(len(u["content"]) for u in current_units)
                else:
    ...
    def _create_chunk(self, units: List[Dict], page: int, chunk_idx: int) -> Chunk:
        ...
        return Chunk(
            ...
            metadata={
                ...
                "has_list": any(
                    t in chunk_types for t in ["numbered_list", "bullet_list"]
                ),
